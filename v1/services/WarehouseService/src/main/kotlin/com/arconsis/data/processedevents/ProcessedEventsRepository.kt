...
class ProcessedEventsRepository(private val sessionFactory: Mutiny.SessionFactory) {
    fun createEvent(event: ProcessedEvent, session: Session? = null): Uni<ProcessedEvent> {
        return session?.createEvent(event) ?: createEvent(event)
    }

    private fun Session.createEvent(event: ProcessedEvent): Uni<ProcessedEvent> {
    ...
    fun getEvent(eventId: UUID, session: Session): Uni<ProcessedEvent?> {
